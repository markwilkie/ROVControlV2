...
namespace ROVControl
{
    public class ControlLoop
    {
        ...
        public bool stopSignal = true;

        //Constants for tick control
        const int SLEEP_TICKS = 2;
        const int CRAFT_TICKS = 0;

        //Current ticks
        int currentCraftTick = 0;

        //Form 
        frmMain mainFrm;

        //Start and stops loop with return 0 of stopped, and 1 for started
        public void Stop()
        {
            //If running, set stop flag to stop it, otherwise ignore
            if (running)
            {
                stopSignal = true;
            }
        ...
        public void Start(frmMain form)
        {
            //Set flags before we start the loop
            running = true;
            stopSignal = false;
            ...
            while (true)
            {
                ...
                if (Win32.PeekMessage(out msg, mainFrm.Handle, 0, 0, (uint)Win32.PM.REMOVE))
                {
                ...
                else
                {
                    ...
                    // Get input
                    mainFrm.UpdateStick_Tick();
                    mainFrm.UpdateKeyboard_Tick();

                    //Move and get status from craft
                    ManageCraft();

                    // Give the processor some space
                    System.Threading.Thread.Sleep(SLEEP_TICKS);
                    ...
                    //if (ms > maxTime)
                    //    maxTime = ms;
                    //mainFrm.UpdateStopWatchLabel(maxTime.ToString());
                }
            }
        }

        private void ManageCraft()
        {
            //Increment Ticks
            currentCraftTick++;

            // Move ROV every n ticks
            if (currentCraftTick >= CRAFT_TICKS)
            {
                mainFrm.MoveCraft_Tick();
                mainFrm.SendReceiveSerial_Tick();
                mainFrm.UpdateSensorStatus_Tick();
                currentCraftTick = 0;
            }
        }
    }
